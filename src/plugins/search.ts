...
export default class Plugin extends BasePlugin {
  ...
  render = async () => {
    ...
    const modal = `
      <input type="search" placeholder="${options.placeholder}" value="${defaultValue}" />
      <ul></ul>
      <div class="help">
        <ul class="d-flex">
        ${window.$tags && `<li onclick="$modal.close();$modal.open('tags.modal')" class="cursor">
          <kbd><i class="bx bx-hash"></i></kbd>
          <span>tags</span>
        </li>` || ''}  
        <li>
    ...
    const button = `<button type="button" class="contrast outline">
      <span class="d-flex align-items-center">
        ...
        <span class="text">${options.placeholder}</span>
      </span>
      <span><kbd><span class="symbol">${s1}</span>${s2}</kbd></span>
    </button>`;

    const open = () => ui.get("modal").open();
  ...
  bind = async () => {
    ...
    const highlight = (terms: string[], str: string) => {
      ...
      return str.replace(
        re,
        (matched: string) => `<mark class='highlight'>${matched}</mark>`
      );
    };
    ...
    const makeSearch = () => {
      ...
      if (value && value.length >= options.chars) {
        ...
        searchResults.map((r: ObjectAny, i: number) => {
          ...
          text = text.replace("<a/>", utils.w.encodeToHtml("<a/>"));
    ...
    hotkeys?.register(
      ...
      "open global search",
      function (event: any) {
        modal.el.prop("open") ? modal.close() : modal.open();
      }
    );
  };
  ...
  style = async () => `
    [id="search.modal"] {
      ...
      .highlight {
        ...
        border-bottom: 2px solid var(--mark-background-color);
      }
